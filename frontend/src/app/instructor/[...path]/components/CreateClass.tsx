"use client"
import { Text, Heading, Separator, IconButton, Button, Checkbox, Spinner } from '@radix-ui/themes';
...
import { DollarSignIcon, CalendarIcon, Clock, ArrowRight, UsersRoundIcon, Repeat, MapPinIcon, CalendarCogIcon, WaypointsIcon, SendIcon, EllipsisVerticalIcon, InfinityIcon } from 'lucide-react';
...
export default function CreateClass({ class_id, userId }: MyClassesProps) {
    ...
    useEffect(() => {
        async function fetchClass() {
            ...
            try {
                ...
                setSearchValue(courseInfo.address);
                setMinAge(Number(courseInfo.minimum_age));
                setTimeSlots([...courseInfo.section]);

                const f1 = courseInfo.image_1_Link;
                const f2 = courseInfo.image_2_Link;

                // Create the promises
                const img1Promise = axios.get(
                    `${process.env.NEXT_PUBLIC_HOSTNAME}/cloud/download/${f1}`,
                    { responseType: 'blob' }
                );
                const img2Promise = axios.get(
                    `${process.env.NEXT_PUBLIC_HOSTNAME}/cloud/download/${f2}`,
                    { responseType: 'blob' }
                );

                // Wait for both promises to resolve and get the responses
                const [img1Response, img2Response] = await Promise.all([img1Promise, img2Promise]);

                // Now create File objects using the response data
                const file1 = new File(
                    [img1Response.data],
                    f1,
                    { type: img1Response.headers['content-type'] || 'image/jpeg' }
                );
                const file2 = new File(
                    [img2Response.data],
                    f2,
                    { type: img2Response.headers['content-type'] || 'image/jpeg' }
                );

                setThumbnail([file1]);
                setAdditionalPictures([file2]);
            } catch (error) {
                console.log(error);
            }
    ...
    async function handleSubmit() {
        ...
        if (class_id) {
            try {
                ...
                // const t_id = res.data[0].id;

                const upd = await axios.put(`${process.env.NEXT_PUBLIC_HOSTNAME}/course/update?course_id=${class_id}`, {
                    course_Name: title,
    ...
    if (classLoading && class_id) {
        return (
            <div className='h-full w-full overflow-y-scroll mt-4'>
                <div className='h-full px-6 md:px-8 flex flex-col max-w-[1024px] bg-white relative items-center justify-center'>
                    <Spinner />
                </div>
            </div>
        )
                ...
                <div className='h-full px-6 md:px-8 flex flex-col max-w-[1024px] bg-white relative'>
                    <div className="md:flex items-center justify-between max-w-[1024px] w-full sticky top-0 bg-white z-10 py-6 border-b border-gray-200 hidden">
                        ...
                        <div className='flex gap-2 flex-col'>
                            ...
                            <Button color='red' variant='solid' className='flex items-center justify-start' onClick={() => router.push("/instructor/classes")}>
                    ...
                    <div className='md:hidden flex items-center max-w-[1024px] w-full sticky top-0 bg-white z-10 py-4 border-b border-gray-200'>
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger className='p-2 flex items-center justify-center rounded-full hover:bg-gray-300/30'>
                            ...
                            <DropdownMenuSeparator /> */}
                                ...
                                <DropdownMenuItem className='flex items-center justify-start' onClick={() => router.push("/instructor/classes")}>
                                    <Trash2Icon width={16} height={16} />
                                    Delete
                                </DropdownMenuItem>
                            </DropdownMenuContent>
                    ...
                    <div className='flex flex-col pb-8'>
                        ...
                        <div className='flex gap-2 mt-2 overflow-x-auto'>
                            <AddTimeSlot timeSlots={timeSlots} setTimeSlots={setTimeSlots} onAdd={async (ts: TimeSlot) => {
                                const old = timeSlots;
                                const slots = [...timeSlots, ts];
                                setTimeSlots(slots);
                            ...
                            {timeSlots.map((timeSlot, i) => (
                                <ViewTimeSlot key={i} timeSlot={timeSlot} idx={i} handleDelete={async () => {
                                    const old = timeSlots;
                                    ...
                                    if (class_id) {
                                        try {
                                            await axios.delete(`${process.env.NEXT_PUBLIC_HOSTNAME}/timeslot/SOFTdelete/${timeSlot.id}`)
            ...
            {timeSlot.rrule ? (
                <div className='flex flex-col'>
                    ...
                    <div className='flex gap-1 items-center'>
                        ...
                        <Text size={"1"}>
                            {timeSlot.dtend && new Date(timeSlot.dtend) > new Date(new Date().setFullYear(new Date().getFullYear() + 10)) ?
                                <InfinityIcon width={12} height={12} />
                                :
...
function AddTimeSlot({ timeSlots, setTimeSlots, onAdd }: { timeSlots: TimeSlot[], setTimeSlots: React.Dispatch<React.SetStateAction<TimeSlot[]>>, onAdd: (ts: TimeSlot) => Promise<void> }) {
    return (
        <TimeSlotPopup timeSlots={timeSlots} setTimeSlots={setTimeSlots} onAdd={onAdd}>
